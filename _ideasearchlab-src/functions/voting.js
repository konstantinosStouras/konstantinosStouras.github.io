...
exports.submitVote = functions.https.onCall(async (data, context) => {
  ...
  if (!sessionId || !Array.isArray(ideaIds) || ideaIds.length < 1) {
    throw new HttpsError('invalid-argument', 'Must provide at least 1 idea ID.')
  }
  ...
  if (!groupId) throw new HttpsError('failed-precondition', 'Participant has no group.')
  if (participant.votedFor) throw new HttpsError('already-exists', 'Already voted.')
