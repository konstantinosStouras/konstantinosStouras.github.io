...
export default function DataAnalytics() {
    ...
    reader.onload = ev => {
      try {
        ...
        for (const [ciNov, ciUse, target] of pairs) {
          ...
          const one = matchScoresIntoRows(res.rows, entries, r => !isExcludedRow(r), target)
  ...
  // "Download all data" (owner, 2026-09-24: "there is no download button that
  ...
  function downloadAllData() {
    ...
    addSheet(wb, 'ideas', ideaExportRows(data))
  ...
  function downloadAllDataCsv() {
    const data = effectiveRows
    if (!data.length) return
    const objs = ideaExportRows(data)
  ...
  // The same idea sheet as "Download all data" (so the two files never disagree),
            ...
            <>
              ...
              <div className={styles.row} style={{ marginTop: 14, alignItems: 'center', gap: 10 }}>
                ...
                <span className={styles.hint} style={{ margin: 0 }}>
                  ...
                  The Excel file adds a <em>Usefulness score check</em> sheet (each idea&apos;s three parts, their ranks and the
                  mean), summaries by condition and by session, and the pool KPIs.
                </span>
              </div>
...
function addUsefulnessCheckSheet(wb, data, techSet) {
  ...
  const body = rows.map((r, i) => {
    ...
    const note = stale.length
      ...
      : ''
    return [
      r.idea_id, r.condition, r.idea_title || String(text).split(': ')[0],
