...
import { doc, updateDoc, onSnapshot, serverTimestamp, db } from '../utils/db'
...
export default function Survey() {
  ...
  const c = getContent(session).survey
  ...
  visibleQuestions.forEach(q => {
    num++
    if (q.section) {
      current = { title: q.section, subtitle: q.sectionSubheading || '', questions: [] }
      sections.push(current)
    }
    if (!current) {
      current = { title: null, subtitle: '', questions: [] }
      sections.push(current)
    }
            ...
            <div key={si} className={styles.sectionCard}>
              {sec.title && (
                <div className={styles.sectionHeading}>
                  {sec.title}
                  {sec.subtitle && (
                    <div className={styles.sectionSubheading}>{sec.subtitle}</div>
                  )}
                </div>
              )}
                      ...
                      <div className={styles.scaleWrap}>
                        <div className={styles.boxScale}>
                          {[1, 2, 3, 4, 5].map(n => (
                            <label
                              key={n}
                              className={`${styles.scaleBox} ${answers[q.id] === n ? styles.scaleBoxActive : ''}`}
                            >
                              <input
                                type="radio"
                                name={q.id}
                              ...
                              />
                              <span className={styles.scaleBoxNum}>{n}</span>
                            </label>
                          ))}
                        </div>
                        ...
                        {q.items.map(item => {
                          ...
                          return (
                            <div key={item.id} className={styles.ratingItem}>
                              <div className={styles.ratingItemLabel}>
                                {item.label}
                                {item.description && (
                                  <span className={styles.ratingItemDesc}> — {item.description}</span>
                                )}
                              </div>
                              <div className={styles.scaleWrap}>
                                <div className={styles.boxScale}>
                                  {[1, 2, 3, 4, 5].map(n => (
                                    <label
                                      key={n}
                                      className={`${styles.scaleBox} ${ga[item.id] === n ? styles.scaleBoxActive : ''}`}
                                    >
                                      <input
                                        type="radio"
                                        name={`${q.id}_${item.id}`}
