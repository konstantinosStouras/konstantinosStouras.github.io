...
- **Analysis-ready / condition-coded export (for AsPredicted #298152, "The Effects of AI Timing on Idea Generation and Selection" — the 2×2 AI-timing study; the idea is the unit of analysis):** the export is shaped so single-session files **stack into one condition-coded dataset**. A `stamp()` helper prepends four columns to **every** data sheet (all sheets except the static "AI Pricing"): **Session Code**, **AI Condition** (one of the four pre-registered labels — `Human-Only Hybrid (no AI)`, `Individual + AI (AI in solo stage only)`, `Group + AI (AI in group stage only)`, `Full AI (AI in both stages)`, derived from `aiConfig.individualAI`/`groupAI` gated by phase-active), **AI Solo Stage** (Yes/No), **AI Group Stage** (Yes/No). The condition for LONDON-style sessions can also be inferred from AI usage (e.g. 0 solo prompts + group prompts ⇒ Group + AI).
...
- **Sheet 1 -- Participants**: ID, name, email, anonymous label, group ID, status, individual complete, votes submitted, voted for (comma-separated IDs), consent, demographics (all fields incl. **Age/Gender controls**), joined at — condition-stamped.
- **Sheet 2 -- Ideas (the unit of analysis)**: condition-stamped; one row per idea with **Stage** (`individual (solo)` / `group`), Phase, Group ID (falls back to the author's group for individual ideas, which don't carry a groupId on the doc — gives the triad id for clustering), Author ID/Name/**Author Label** (anonymous), Title, Description, Full Text (for semantic-dispersion / search-breadth measures), **Carried to Group** (renamed from the ambiguous "Selected" — = double-click-selected in the individual phase to carry forward, NOT a final pick), vote count, **Final Group Pick** (Yes/No — is this idea in its group's `finalIdeas`, i.e. one the group locked in after voting), **Final Pick Rank** (e.g. `g2 #1`, top-voted first), created at, and **three empty blind-rater DV columns** — `Novelty (rater)`, `Usefulness (rater)`, `Overall Quality (rater)` (Overall = mean of novelty & usefulness) — so the file doubles as the expert-rating sheet. *(Final Group Pick / Final Pick Rank answer "which ideas did each group select after voting" directly from the Ideas sheet.)*
