...
- **Session details (create form)**: a top "Session details" section (shown only when creating, not editing) with an optional **Session name** (`e.g. Spring MBA 2026`, stored as `session.name` and shown in the Active/Completed session cards) and an optional **Session ID** custom code. The custom code is validated `^[A-Za-z0-9-]{3,40}$`, stored upper-cased (JoinSession upper-cases participant input, so codes match case-insensitively while preserving dashes), and checked for uniqueness via a `getDocs(where('code','==',code))` query before `addDoc`. Blank ID falls back to the auto-generated short code. JoinSession's code input was widened to `maxLength={40}` / min length 3 to accept custom codes.
