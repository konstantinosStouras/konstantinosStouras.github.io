...
- **Sheet 2 -- Ideas**: ID, title, description, full text, author, phase, group ID (falls back to the author's group for individual ideas, which don't carry a groupId on the doc), **Carried to Group** (renamed from the ambiguous "Selected" — = double-click-selected in the individual phase to carry forward, NOT a final pick), vote count (tallied from participants' votedFor arrays), **Final Group Pick** (Yes/No — is this idea in its group's `finalIdeas`, i.e. one the group locked in after voting), **Final Pick Rank** (e.g. `g2 #1`, top-voted first), created at. *(The Final Group Pick / Final Pick Rank columns answer "which ideas did each group actually select after voting" directly from the Ideas sheet — previously you had to cross-reference raw ids in the Groups sheet.)*
