...
- **Live group control (during play)**: each group header has **View** and **Message** buttons, and each participant row has a **Remove** button (two-click confirm).
  ...
  - **Message** (`sendGroupMessage`): writes `adminMessage = { id: Date.now(), text, from }` to every group member's participant doc (instructors already have participant-update permission). `AdminBroadcast` (`src/components/AdminBroadcast.jsx`, mounted once in `SessionWrapper` so it rides over every session page) subscribes to the signed-in participant's own doc and pops the message up as a **centred window**; dismissing records `adminMessageAckId` (a newer message id shows again).
  - **Remove** (`removeParticipant` Cloud Function): detaches a participant from their group mid-session even after play has started. It marks them `removed: true` / `status: 'removed'` / `groupId: null`, removes them from the group doc's `members`/`memberLabels`, and — if the group is still active — pushes a vacancy `{ groupId, label, phase }` onto `session.backfillQueue`. `reconcileGroupAfterRemoval` then advances the group if the removed member was the only one still blocking it (mirrors autoGroupParticipants / finishGroupVoting for the survivors). The removed participant sees a full-screen "you've left this session" overlay (also via AdminBroadcast).
