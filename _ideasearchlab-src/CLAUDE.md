...
**Split-screen UI:** main app on left, AI chat on right, draggable divider. When AI is off the left panel fills full width. The AI chat input (AIChat.jsx) **auto-grows** with its content — height is set in JS from the textarea's `scrollHeight` on every change (min-height 52px, max-height 240px then scrolls), so a long message stays fully visible instead of scrolling inside a 2-row box. The input is **never disabled while the AI is thinking**, so participants can keep typing their next question during a reply; submitting is still gated on `sending` (send button + `handleKeyDown`) so requests don't overlap.
