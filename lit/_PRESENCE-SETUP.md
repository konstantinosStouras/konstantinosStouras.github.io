...
The Data Analytics page (`lit/analytics/`) can show a live **Exploring now**
...
- **The main browser (`lit/index.html`) only *writes* presence** — it never
...
- **`lit/index.html`** — the presence-register `<script>` near the very
  bottom (`PRESENCE_CFG.databaseURL`).
- **`lit/analytics/index.html`** — the `RTDB_URL` constant in the Firebase
...
The rules live in **`lit/_database.rules.json`** (public read of `/presence`
...
appears on `lit/analytics/` and updates live as visitors come and go.
