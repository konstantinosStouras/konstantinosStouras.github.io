...
//
//   LIT_DB=/tmp/lit.db BROWSER=/path/to/chromium node lit/_scraper/sqlite-bench.mjs
//
...
function resolveUrl(u) {
  if (u === '/lit/data/lit.db') return DB;
  if (u === '/lit/data/lit.db.length') return DB + '.length';
  if (u.startsWith('/lit/data/db/')) return path.join(CHUNK_DIR, u.slice('/lit/data/db/'.length));
...
await page.goto('http://localhost:' + PORT + '/lit/db-preview/', { waitUntil: 'domcontentloaded' });
