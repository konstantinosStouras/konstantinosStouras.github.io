...
export default function CosineKnapsackGame() {
  ...
  items.forEach((item, idx) => {
    itemColumns[`Item ${idx + 1} Value`] = item.value;
    itemColumns[`Item ${idx + 1} Attributes`] = `[${item.attributes.map(v => v.toFixed(2)).join(", ")}]`;
    itemColumns[`Item ${idx + 1} Selected`] = selectedIds.includes(item.id);
  });
  ...
  const rawData = {
    ...
    browser: navigator.userAgent || window.navigator.userAgent || '',
    ...
    finalSelection: selectedItems.map(i => i.name).join(", "),
    optimalSet: result.subset.map(i => i.name).join(", "),
    optimalValue: result.value,
    optimalSimilarity: averageSimilarity(result.subset),
    ...itemColumns
  };

  const orderedKeys = [
    'userId', 'sessionId', 'timestamp', 'browser', 'device', 'round',
    'totalValue', 'similarity', 'targetSimilarity', 'success',
    'strategy', 'finalSelection', 'optimalSet', 'optimalValue', 'optimalSimilarity',
  ...
  for (const key of orderedKeys) {
    orderedLogData[key] = rawData[key] !== undefined ? rawData[key] : '';
  }
  ...
  const nextRound = () => {
    ...
    try {
      fetch('https://knapsack-dependent-proxy-2.vercel.app/api/submit', {
        method: 'POST',
