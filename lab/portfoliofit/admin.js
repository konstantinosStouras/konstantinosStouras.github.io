...
(function () {
  ...
  var PF_DA_TPL_VERSION = '2026-07-07-deciles-objective-test';
  ...
  var DA_PY_TEMPLATE = [
    ...
    'fig.tight_layout()',
    '',
    '# Figure 5 - the greedy trap: $/cell of the k-th brick placed, top vs rest',
    'fig, ax = plt.subplots(figsize=(7, 4))',
    'adds["top"] = adds["player"].map(top_strict)',
    'for v, c, lab in ((1, BLUE, "top"), (0, GREY, "rest")):',
    '    g = adds[(adds["top"] == v) & (adds["k"] <= 6)].groupby("k")["vpc"]',
    '    ax.errorbar(g.mean().index, g.mean(), yerr=g.std(ddof=1) / np.sqrt(g.count()), color=c, marker="o", capsize=3, label=lab)',
    ...
    'print("Mean $ value per cell of the 1st, 2nd, ... 6th brick placed - a brick\'s Brick Value divided by "',
    ...
    '      "cell - one of the clearest heuristics separating the groups.")',
  ...
  var DA_R_TEMPLATE = [
    ...
    'm5 <- sapply(1:6, function(k) c(mean(adds$vpc[adds$k == k & adds$top == 1], na.rm = TRUE), mean(adds$vpc[adds$k == k & adds$top == 0], na.rm = TRUE)))',
    's5 <- sapply(1:6, function(k) c(sd(adds$vpc[adds$k == k & adds$top == 1], na.rm = TRUE) / sqrt(sum(adds$k == k & adds$top == 1)),',
    '                                sd(adds$vpc[adds$k == k & adds$top == 0], na.rm = TRUE) / sqrt(sum(adds$k == k & adds$top == 0))))',
    'plot(1:6, m5[1, ], type = "b", pch = 19, col = BLUE, ylim = range(c(m5 - s5, m5 + s5), na.rm = TRUE, finite = TRUE), xaxt = "n",',
    '     xlab = "k-th brick placed", ylab = "mean $ value per cell of that brick",',
    '     main = "Do players chase the pricey-per-cell (trap) bricks first?")',
    'axis(1, at = 1:6)',
    'lines(1:6, m5[2, ], type = "b", pch = 19, col = GREY)',
    'arrows(1:6, m5[1, ] - s5[1, ], 1:6, m5[1, ] + s5[1, ], angle = 90, code = 3, length = 0.03, col = BLUE)',
    'arrows(1:6, m5[2, ] - s5[2, ], 1:6, m5[2, ] + s5[2, ], angle = 90, code = 3, length = 0.03, col = GREY)',
    ...
    'cat("Mean $ value per cell of the 1st, 2nd, ... 6th brick placed - a brick\'s Brick Value divided by the number of cells it occupies. Two terms NOT to confuse with the on-screen KPIs: this $/cell is a PER-BRICK price density, not the Value/Resource (ROI) KPI (which divides the whole portfolio\'s Total Value by the empty-cell penalty); and \'completing the board\' below means GEOMETRIC fit, not the Portfolio Fitness compactness KPI. The game is built so the bricks with the highest $/cell are traps: a grey (rest) line starting high and falling = ratio-greedy ordering (grab the priciest-per-cell bricks first), while a flatter blue (top) line = top players picking bricks for how well they complete the board rather than for their price per cell - one of the clearest heuristics separating the groups.\\n")',
