...
(function () {
  ...
  var PF_DA_TPL_VERSION = '2026-07-08-fig5-all-bricks';
  ...
  var DA_PY_TEMPLATE = [
    ...
    '  6. Eight figures - shown in the \'Insights gained\' section, each under the',
    '     paragraph that explains it - and a data-driven INSIGHTS write-up that ENDS',
    ...
    '    return None',
    '',
    'for df, need in ((moves, ["Player", "Session", "Puzzle", "Action", "Move #", "Brick Value", "Cells", "Duration (s)", "Net Value (before)", "Net Value (after)", "Coverage % (before)", "Coverage % (after)", "Portfolio Fitness (before)", "Portfolio Fitness (after)"]),',
    ...
    'mv["bin10"] = np.minimum(np.ceil(mv["progress"] * 10), 10).astype(int)      # ... and into tenths (Figure 6)',
    ...
    'D5 = {}   # the same under the within-5% definition - Figure 7',
    ...
    '    return f"{x:>{w}.3f}" if pd.notna(x) else " " * (w - 3) + "---"',
    'for binc, word, ref in (("bin", "fifth", "Figures 2-3"), ("bin10", "tenth", "Figure 6")):',
    ...
    '',
    'def traj(flag, title):',
    '    """Figure 2/3 body: mean balanced-KPI score per fifth of play, top vs rest,',
    ...
    '    fig.tight_layout()',
    '',
    '# Figure 2 - the balanced-KPI score during play, strict top definition',
    'traj("top_strict", "Balanced-KPI score during play - top (reached max) vs rest")',
    '# Figure 3 - the same with the relaxed (within-5%) definition',
    'traj("top_5pct", "Balanced-KPI score during play - top (within 5% of max) vs rest")',
    '',
    '# Figure 4 - WHY balance differs: the three objectives separately, top vs rest',
    ...
    'fig.tight_layout()',
    '',
    '# Figure 5 - the greedy trap: $/cell of the k-th brick placed (ALL placements),',
    '# top vs rest. Later placements exist only for players who made that many, so',
    '# their error bars naturally widen along the tail.',
    'fig, ax = plt.subplots(figsize=(7, 4))',
    'adds["top"] = adds["player"].map(top_strict)',
    'kmax = int(adds["k"].max())',
    'for v, c, lab in ((1, BLUE, "top"), (0, GREY, "rest")):',
    '    g = adds[adds["top"] == v].groupby("k")["vpc"]',
    '    ax.errorbar(g.mean().index, g.mean(), yerr=g.std(ddof=1) / np.sqrt(g.count()), color=c, marker="o", capsize=3, label=lab)',
    'ax.set_xticks(range(1, kmax + 1))',
    'ax.set_xlabel("k-th brick placed"); ax.set_ylabel("mean $ value per cell of that brick")',
    'ax.set_title("Do players chase the pricey-per-cell (trap) bricks first?")',
    'ax.legend()',
    'fig.tight_layout()',
    '',
    '# Figure 6 - the balanced-KPI trajectories again, at 10% (decile) resolution',
    'fig, axs = plt.subplots(1, 2, figsize=(10, 4), sharey=True)',
    'for ax, flag, ttl in ((axs[0], "top_strict", "top = reached max"), (axs[1], "top_5pct", "top = within 5% of max")):',
    '    for v, c, lab in ((1, BLUE, "top"), (0, GREY, "rest")):',
    '        g = bal[bal[flag] == v].groupby("bin10")["kpi_avg"]',
    ...
    'fig.tight_layout()',
    '',
    '# Figure 7 - the heuristic fingerprint again, under the within-5% definition',
    'fig, ax = plt.subplots(figsize=(7, 4))',
    'y = np.arange(len(MARKERS))',
    ...
    'fig.tight_layout()',
    '',
    '# Figure 8 - how fast each candidate objective climbs per group (Section 5)',
    'fig, axs = plt.subplots(1, 2, figsize=(10, 4), sharey=True)',
    ...
    '      "pricey-per-cell trap bricks (check its sign in the tables and in Figure 5).")',
    'print("## Easy vs hard")',
    ...
    'print("## Figure 7 - the same fingerprint under the within-5% definition")',
    ...
    'print("## Figure 2 - the balanced-KPI score during play (strict top definition)")',
    ...
    'print("## Figure 3 - the same picture with the relaxed (within-5%) definition")',
    'print("If this looks like Figure 2, the finding is not an artefact of the strict cut-off: players who "',
    ...
    'print("## Figure 6 - the balanced-KPI score at 10% resolution")',
    'print("The same trajectories as Figures 2-3 but per TENTH of the play instead of fifths - left panel: "',
    ...
    'print("## Figure 4 - the three objectives separately (what drives the score)")',
    ...
    'print("## Figure 5 - the greedy trap in one picture")',
    ...
    '      "the players who made that many, so the error bars naturally widen along the tail.")',
    ...
    'print("## Figure 8 - how fast each candidate objective climbs, per group")',
    ...
    '        print(f"- OBJECTIVE TEST: by per-move climb rates (Section 5, Figure 8), the best players look {w}.{rel}")',
  ...
  var DA_R_TEMPLATE = [
    ...
    '#             diversified play, (6) eight figures + a data-driven INSIGHTS',
    ...
    '  NA_character_',
    '}',
    'for (need in list(list(moves, c("Player", "Session", "Puzzle", "Action", "Move #", "Brick Value", "Cells", "Duration (s)", "Net Value (before)", "Net Value (after)", "Coverage % (before)", "Coverage % (after)", "Portfolio Fitness (before)", "Portfolio Fitness (after)")),',
    ...
    'mv$bin10 <- pmin(ceiling(mv$progress * 10), 10)   # ... and into tenths (Figure 6)',
    ...
    'D5 <- list()   # the same under the within-5% definition - Figure 7',
    ...
    '# Format one table number to width w, or \'---\' when the cell is empty',
    '# (e.g. a tenth of play in which one group happens to have no moves).',
    'cell <- function(x, w) if (is.finite(x)) sprintf(paste0("%", w, ".3f"), x) else sprintf(paste0("%", w, "s"), "---")',
    'for (spec in list(c("bin", "fifth", "Figures 2-3"), c("bin10", "tenth", "Figure 6"))) {',
    ...
    '}',
    'traj("top_strict", "Balanced-KPI score during play - top (reached max) vs rest")   # Figure 2',
    'traj("top_5pct",  "Balanced-KPI score during play - top (within 5% of max) vs rest")   # Figure 3',
    '',
    '# Figure 4 - WHY balance differs: the three objectives separately, top vs rest',
    'par(mfrow = c(1, 2))',
    'for (v in c(1, 0)) {',
    ...
    'kmax <- max(adds$k)',
    'm5 <- sapply(1:kmax, function(k) c(mean(adds$vpc[adds$k == k & adds$top == 1], na.rm = TRUE), mean(adds$vpc[adds$k == k & adds$top == 0], na.rm = TRUE)))',
    's5 <- sapply(1:kmax, function(k) c(sd(adds$vpc[adds$k == k & adds$top == 1], na.rm = TRUE) / sqrt(sum(adds$k == k & adds$top == 1)),',
    '                                   sd(adds$vpc[adds$k == k & adds$top == 0], na.rm = TRUE) / sqrt(sum(adds$k == k & adds$top == 0))))',
    'plot(1:kmax, m5[1, ], type = "b", pch = 19, col = BLUE, ylim = range(c(m5 - s5, m5 + s5), na.rm = TRUE, finite = TRUE), xaxt = "n",',
    '     xlab = "k-th brick placed", ylab = "mean $ value per cell of that brick",',
    '     main = "Do players chase the pricey-per-cell (trap) bricks first?")',
    'axis(1, at = 1:kmax)',
    'lines(1:kmax, m5[2, ], type = "b", pch = 19, col = GREY)',
    'arrows(1:kmax, m5[1, ] - s5[1, ], 1:kmax, m5[1, ] + s5[1, ], angle = 90, code = 3, length = 0.03, col = BLUE)',
    'arrows(1:kmax, m5[2, ] - s5[2, ], 1:kmax, m5[2, ] + s5[2, ], angle = 90, code = 3, length = 0.03, col = GREY)',
    'legend("topright", c("top", "rest"), col = c(BLUE, GREY), pch = 19, lty = 1, bty = "n")',
    '',
    '# Figure 6 - the balanced-KPI trajectories again, at 10% (decile) resolution',
    'par(mfrow = c(1, 2))',
    'for (flag in c("top_strict", "top_5pct")) {',
    '  m <- sapply(1:10, function(i) c(mean(bal$kpi_avg[bal$bin10 == i & bal[[flag]] == 1]), mean(bal$kpi_avg[bal$bin10 == i & bal[[flag]] == 0])))',
    '  s <- sapply(1:10, function(i) c(sd(bal$kpi_avg[bal$bin10 == i & bal[[flag]] == 1]) / sqrt(sum(bal$bin10 == i & bal[[flag]] == 1)),',
    '                                  sd(bal$kpi_avg[bal$bin10 == i & bal[[flag]] == 0]) / sqrt(sum(bal$bin10 == i & bal[[flag]] == 0))))',
    '  plot(1:10, m[1, ], type = "b", pch = 19, col = BLUE, ylim = range(c(m - s, m + s), na.rm = TRUE, finite = TRUE), xaxt = "n",',
    '       xlab = "tenth of the play (1 = first moves, 10 = last)", ylab = "mean balanced-KPI score",',
    ...
    'cat("- Reading the markers as HEURISTICS: n_moves = exploration volume; rem_share = treating placements as reversible experiments (every removal is a deliberate, temporary net sacrifice); early_vpc = ratio-chasing - a NEGATIVE top-minus-rest gap means top players resist the pricey-per-cell trap bricks (check its sign in the tables and in Figure 5).\\n")',
    'cat("## Easy vs hard\\n")',
    ...
    'cat("## Figure 7 - the same fingerprint under the within-5% definition\\n")',
    ...
    'cat("## Figure 2 - the balanced-KPI score during play (strict top definition)\\n")',
    ...
    'cat("## Figure 3 - the same picture with the relaxed (within-5%) definition\\n")',
    'cat("If this looks like Figure 2, the finding is not an artefact of the strict cut-off: players who merely came close to the maximum share the balanced style. If the gap disappears here, \'balance\' distinguishes only the very best plays.\\n")',
    'cat("## Figure 6 - the balanced-KPI score at 10% resolution\\n")',
    'cat("The same trajectories as Figures 2-3 but per TENTH of the play instead of fifths - left panel: top = reached the maximum; right panel: top = within 5% of it. The finer grid shows WHEN the groups separate (right from the opening moves, or only in the endgame); bars are +-1 SE, and a tenth in which a group happens to have no moves is simply skipped. The tables behind both resolutions are printed in the Section-3 output.\\n")',
    'cat("## Figure 4 - the three objectives separately (what drives the score)\\n")',
    ...
    '}',
    'cat("## Figure 8 - how fast each candidate objective climbs, per group\\n")',
    ...
    '    cat(sprintf("- OBJECTIVE TEST: by per-move climb rates (Section 5, Figure 8), the best players look %s.%s\\n", w, rel))',
    '  }',
