...
(function () {
  ...
  var PF_DA_TPL_VERSION = '2026-07-07-heuristics-balanced-kpi';
  ...
  var DA_PY_TEMPLATE = [
    ...
    '  5. Five figures - shown in the \'Insights gained\' section, each under the',
    ...
    '    return None',
    '',
    'for df, need in ((moves, ["Player", "Session", "Puzzle", "Action", "Move #", "Brick Value", "Cells", "Duration (s)", "Net Value (before)", "Net Value (after)", "Coverage % (after)", "Portfolio Fitness (after)"]),',
    ...
    '# per-difficulty top flag: reached the maximum on >= 1 puzzle OF THAT difficulty',
    ...
    'D = {}   # Cohen\'s d per (difficulty, marker) - drawn in Figure 1',
    'for d in ("easy", "hard"):',
    '    a_n = int((pl.loc[pl["diff"] == d, "top_d"] == 1).sum())',
    '    b_n = int((pl.loc[pl["diff"] == d, "top_d"] == 0).sum())',
    '    print(f"\\n--- {d.upper()} puzzles: reached the maximum (n={a_n}) vs did not (n={b_n}) ---")',
    '    print(f"{\'marker\':<11}{\'top mean\':>10}{\'rest mean\':>11}{\'diff\':>9}{\'p\':>9}")',
    '    for m in MARKERS:',
    '        a = pl.loc[(pl["diff"] == d) & (pl["top_d"] == 1), m].dropna()',
    '        b = pl.loc[(pl["diff"] == d) & (pl["top_d"] == 0), m].dropna()',
    '        if len(a) > 1 and len(b) > 1 and (a.std(ddof=1) > 1e-12 or b.std(ddof=1) > 1e-12):',
    '            sp = np.sqrt(((len(a) - 1) * a.var(ddof=1) + (len(b) - 1) * b.var(ddof=1)) / (len(a) + len(b) - 2))',
    '            D[(d, m)] = (a.mean() - b.mean()) / sp if sp > 0 else 0.0   # standardised gap',
    '            dv = a.mean() - b.mean()',
    ...
    '',
    'print("\\nMean balanced-KPI score by fifth of play (the data behind Figures 2-3):")',
    'print(f"{\'fifth\':<7}{\'top(strict)\':>12}{\'rest\':>8}{\'top(5%)\':>10}{\'rest\':>8}")',
    'for i in range(1, 6):',
    '    r = [bal.loc[(bal["bin"] == i) & (bal[f] == v), "kpi_avg"].mean() for f, v in (("top_strict", 1), ("top_strict", 0), ("top_5pct", 1), ("top_5pct", 0))]',
    '    print(f"{i:<7}{r[0]:>12.3f}{r[1]:>8.3f}{r[2]:>10.3f}{r[3]:>8.3f}")',
    '',
    ...
    'print("Mean $ value per cell of the 1st, 2nd, ... 6th brick placed. The game is built so the highest "',
    '      "$/cell bricks are traps; a grey (rest) line starting high and falling = ratio-greedy ordering, "',
    '      "while a flatter blue (top) line = value-density ignored in favour of fit - the single clearest "',
    '      "heuristic separating the groups.")',
  ...
  var DA_R_TEMPLATE = [
    ...
    '#             outcome), all with player-clustered CR1 SEs,',
    '#             (5) five figures + a data-driven INSIGHTS write-up (rendered,',
    '#             with the figures, in the \'Insights gained\' section), ending with',
    '#             a VERDICT: are the best players focused on one KPI, or balancing',
    '#             across the KPIs, compared to the rest?',
    '',
    '# ---- 0. load the two sheets the page hands over ------------------------------',
    ...
    '  NA_character_',
    '}',
    'for (need in list(list(moves, c("Player", "Session", "Puzzle", "Action", "Move #", "Brick Value", "Cells", "Duration (s)", "Net Value (before)", "Net Value (after)", "Coverage % (after)", "Portfolio Fitness (after)")),',
    ...
    'D <- list()   # Cohen\'s d per (difficulty, marker) - drawn in Figure 1',
    'for (d in c("easy", "hard")) {',
    '  a_n <- sum(pl$diff == d & pl$top_d == 1); b_n <- sum(pl$diff == d & pl$top_d == 0)',
    '  cat(sprintf("\\n--- %s puzzles: reached the maximum (n=%d) vs did not (n=%d) ---\\n", toupper(d), a_n, b_n))',
    '  cat(sprintf("%-11s%10s%11s%9s%9s\\n", "marker", "top mean", "rest mean", "diff", "p"))',
    '  for (m in MARKERS) {',
    '    a <- pl[pl$diff == d & pl$top_d == 1, m]; a <- a[!is.na(a)]',
    '    b <- pl[pl$diff == d & pl$top_d == 0, m]; b <- b[!is.na(b)]',
    '    if (length(a) > 1 && length(b) > 1 && (sd(a) > 1e-12 || sd(b) > 1e-12)) {',
    '      sp <- sqrt(((length(a) - 1) * var(a) + (length(b) - 1) * var(b)) / (length(a) + length(b) - 2))',
    '      D[[paste(d, m)]] <- if (sp > 0) (mean(a) - mean(b)) / sp else 0   # standardised gap',
    '      dv <- mean(a) - mean(b)',
    ...
    '',
    'cat("\\nMean balanced-KPI score by fifth of play (the data behind Figures 2-3):\\n")',
    'cat(sprintf("%-7s%12s%8s%10s%8s\\n", "fifth", "top(strict)", "rest", "top(5%)", "rest"))',
    'for (i in 1:5) {',
    '  r <- c(mean(bal$kpi_avg[bal$bin == i & bal$top_strict == 1]), mean(bal$kpi_avg[bal$bin == i & bal$top_strict == 0]),',
    '         mean(bal$kpi_avg[bal$bin == i & bal$top_5pct == 1]),  mean(bal$kpi_avg[bal$bin == i & bal$top_5pct == 0]))',
    '  cat(sprintf("%-7d%12.3f%8.3f%10.3f%8.3f\\n", i, r[1], r[2], r[3], r[4]))',
    '}',
    '',
    ...
    'cat("Mean $ value per cell of the 1st, 2nd, ... 6th brick placed. The game is built so the highest $/cell bricks are traps; a grey (rest) line starting high and falling = ratio-greedy ordering, while a flatter blue (top) line = value-density ignored in favour of fit - the single clearest heuristic separating the groups.\\n")',
