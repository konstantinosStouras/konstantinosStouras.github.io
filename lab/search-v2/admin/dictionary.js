...
(function (root, factory) {
  ...
  var ROUNDS = {
    ...
    final_score: ['points', 'The round’s settled prize minus total_cost — under best_found the best prize they held, under nominate the true prize at the position they named. May be negative — there is no floor. A retro-corrected round (score_corrected TRUE) exports its re-settled best_found figure here.'],
    ...
    nomination_type: ['factor', 'Where the round\u2019s prize came from. Under the best_found stop rule: best_revealed (a position they paid to open), best_pre_opened (one open from the start), or nothing_found (they stopped having found none, scoring 0 minus what they spent). Under the legacy nominate rule: verified (they had opened the position they named), queried_only (they had only asked the AI about it), or untouched.'],
    stop_rule: ['factor', 'The settlement rule this row\u2019s EXPORTED score follows: best_found (stopping takes the best prize already found and never opens a new position) or nominate (the legacy rule \u2014 stopping names a position and pays the true prize there). Locked at launch. Sessions stored before the parameter existed settle as best_found (owner decision 2026-08-17 \u2014 stopping must never reveal a new prize or add a cost), and any of their rounds that settled under the old nominate fallback is re-settled at export: see score_corrected.'],
    score_corrected: ['boolean', 'TRUE when this round settled under the legacy nominate fallback inside a best_found session (the 2026-08-17 bug \u2014 pressing Stop opened the selected position for free and priced it into the score) and was RE-SETTLED at export under best_found, from the positions open at the start plus the participant\u2019s own reveals. The exported score/position/type follow the corrected settlement; the as-played ones sit in the as_played_ columns.'],
    as_played_stop_rule: ['factor', 'Only on corrected rounds: the rule the round actually settled under when played (always nominate).'],
    as_played_position: ['integer', 'Only on corrected rounds: the position the settlement actually took when played.'],
    as_played_true_value: ['points', 'Only on corrected rounds: the true prize at that position \u2014 the figure that wrongly entered the round\u2019s objective.'],
    as_played_nomination_type: ['factor', 'Only on corrected rounds: the nomination_type as played (verified / queried_only / untouched).'],
    as_played_score: ['points', 'Only on corrected rounds: the final score the participant was shown when the round was played.'],
    as_played_raw_score: ['points', 'Only on corrected rounds: the as-played score before any floor. Equal to as_played_score in this design.'],
  ...
  var PARTICIPANTS = {
    ...
    total_score: ['points', 'Sum of final_score over the scored rounds. When any round was retro-corrected (rounds_score_corrected > 0) this is the CORRECTED sum; the figure the session once reported is total_score_as_played.'],
    rounds_score_corrected: ['integer', 'How many of this participant’s rounds were re-settled at export under the 2026-08-17 stop-rule correction. 0 for any session unaffected by the bug.'],
    total_score_as_played: ['points', 'Only when rounds were corrected: the running total the session itself reported to the participant, before correction.'],
    ...
    perceived_vs_actual_half: ['boolean', 'Whether the half they believed they did better in is the half they did. Computed from the EXPORTED round scores — corrected where score_corrected is set, so on corrected participants the belief was formed against the as-played figures.'],
    ...
    actual_better_half: ['factor', 'Which half they actually scored higher in, from the EXPORTED round scores (corrected where score_corrected is set).'],
