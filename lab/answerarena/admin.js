...
(function () {
  ...
  var DA_PY_TEMPLATE = [
    '"""',
    'Answer Arena - do people prefer Opus 4.8 (frontier) or Haiku 4.5 (baseline)?',
    '',
    'Each row of the Responses table is one BLIND comparison: a participant saw a',
    'task (a real user need) and two answers - one written by Haiku (baseline), one',
    'by Opus (frontier) - WITHOUT being told which was which, chose the answer they',
    'preferred (or "equally good"), then graded how strongly on a 7-point scale.',
    '',
    'Handed in as the string DATA_CSV. Key columns:',
    '  account_id        participant id (repeated measures -> we cluster on this)',
    '  task_id           the task / user need (the item)',
    '  chosen_model      baseline (Haiku) | frontier (Opus) | tie',
    '  preference_model  -3..+3 graded preference, MODEL frame: <0 Haiku, >0 Opus',
    '  response_ms       decision time',
    ...
    'df["pref"] = pd.to_numeric(df.get("preference_model", np.nan), errors="coerce")',
    'df["chosen"] = df.get("chosen_model", "").astype(str).str.lower()',
    '',
    'n = len(df)',
    'n_part = df["account_id"].nunique() if "account_id" in df.columns else float("nan")',
    'n_task = df["task_id"].nunique() if "task_id" in df.columns else float("nan")',
    'print("=" * 72)',
    'print("ANSWER ARENA - blind preference: Haiku 4.5 (baseline) vs Opus 4.8 (frontier)")',
    'print("=" * 72)',
    'print("Comparisons: %d   Participants: %s   Tasks: %s" % (n, n_part, n_task))',
    '',
    'if n == 0:',
    '    print("\\nNo comparisons in the selected table - load the Responses table in Section 1.")',
    'else:',
    '    # ── TABLE 1 - overall outcome shares ──────────────────────────────────────',
    '    opus = int((df["chosen"] == OPUS).sum())',
    '    haiku = int((df["chosen"] == HAIKU).sum())',
    '    ties = int((df["chosen"] == "tie").sum())',
    '    decisive = opus + haiku',
    '    win = opus / decisive if decisive else float("nan")',
    '    t1 = pd.DataFrame({"outcome": ["Opus preferred", "Haiku preferred", "Equally good (tie)"],',
    '                       "n": [opus, haiku, ties],',
    '                       "share_%": [100 * opus / n, 100 * haiku / n, 100 * ties / n]})',
    '    print("\\nTABLE 1 - Overall outcome shares")',
    '    print(t1.to_string(index=False, float_format=lambda x: "%.1f" % x))',
    '    p_bin = float("nan")',
    '    if decisive:',
    '        p_bin = st.binomtest(opus, decisive, 0.5).pvalue',
    '        print("\\nAmong the %d decisive comparisons (ties dropped), Opus was preferred %d times "',
    '              "= %.1f%%.\\nTwo-sided binomial test vs an even 50/50 split: p = %.4g."',
    '              % (decisive, opus, 100 * win, p_bin))',
    '',
    '    # ── THE MAIN QUESTION - are users indifferent, or do they prefer a model? ──',
    '    # H0: users are INDIFFERENT (mean graded preference = 0).',
    '    # H1: they prefer one model (mean != 0). Positive => Opus, negative => Haiku.',
    '    # We answer it two ways: (A) averaging ACROSS TASKS - each task\'s mean is one',
    '    # observation, so tasks with more comparisons don\'t dominate (the headline',
    '    # test the study asks for); and (B) across all comparisons, clustered on the',
    '    # participant (repeated measures). Plus a distribution-free sign test.',
    '    reg = df.dropna(subset=["pref"]).copy()',
    '',
    '    # (A) Task-level: mean each task, then one-sample t-test of the task means vs 0.',
    '    mtm = p_task = t_task = tlo = thi = float("nan"); k_tasks = 0',
    ...
    '    t2 = pd.DataFrame({',
    '        "test": ["(A) averaging across tasks (task = unit)", "(B) all comparisons, clustered on participant"],',
    '        "n": [k_tasks, len(reg)],',
    '        "mean_pref": [mtm, mean],',
    '        "95%_CI": ["[%.3f, %.3f]" % (tlo, thi) if k_tasks >= 2 else "n/a",',
    '                    "[%.3f, %.3f]" % (lo, hi) if len(reg) else "n/a"],',
    '        "p(mean=0)": [p_task, p_mean],',
    '    })',
    '    print(t2.to_string(index=False, float_format=lambda x: "%.4f" % x))',
    '    if k_tasks >= 2:',
    '        print("  (A) one-sample t-test across %d tasks: t(%d) = %.3f, p = %.4g" % (k_tasks, k_tasks - 1, t_task, p_task))',
    '    if pos + neg:',
    '        print("  Sign test across tasks: %d favour Opus, %d favour Haiku (two-sided p = %.4g)" % (pos, neg, p_sign))',
    '',
    '    # ── TABLE 3 - regression on the 2x2 factors (if they were varied) ─────────',
    '    factors = [c for c in ["cost_transparency", "firm_pay"] if c in df.columns]',
    '    for c in factors:',
    ...
    '        tt = pd.DataFrame({',
    '            "n": g.size(),',
    '            "opus": g["chosen"].apply(lambda s: (s == OPUS).sum()),',
    '            "haiku": g["chosen"].apply(lambda s: (s == HAIKU).sum()),',
    '            "tie": g["chosen"].apply(lambda s: (s == "tie").sum()),',
    '            "mean_pref": g["pref"].mean(),',
    '        })',
    '        dec = (tt["opus"] + tt["haiku"]).replace(0, np.nan)',
    '        tt["opus_win_%"] = 100 * tt["opus"] / dec',
    '        tt = tt.sort_values("mean_pref", ascending=False)',
    '        tasks_total = len(tt)',
    '        tasks_favouring_opus = int((tt["mean_pref"] > 0).sum())',
    '        tasks_favouring_haiku = int((tt["mean_pref"] < 0).sum())',
    '        print("\\nTABLE 4 - By task  (sorted by mean preference; >0 favours Opus)")',
    '        show = tt if len(tt) <= 20 else pd.concat([tt.head(10), tt.tail(10)])',
    '        print(show.to_string(float_format=lambda x: "%.2f" % x))',
    '',
    '    # ── PLOTS ─────────────────────────────────────────────────────────────────',
    '    col = lambda v: "#e67e22" if v > 0 else ("#3d7bd6" if v < 0 else "#9a978f")',
    '',
    '    # Figure 1: graded-preference distribution + outcome shares',
    '    fig, ax = plt.subplots(1, 2, figsize=(11, 4.4))',
    '    pref_all = df["pref"].dropna()',
    '    counts = [int((pref_all == v).sum()) for v in range(-3, 4)]',
    '    ax[0].bar(range(-3, 4), counts, color=[col(v) for v in range(-3, 4)])',
    '    ax[0].set_xticks(range(-3, 4))',
    '    ax[0].set_xlabel("graded preference  (-3 Haiku .. +3 Opus)", fontsize=11)',
    '    ax[0].set_ylabel("comparisons", fontsize=11)',
    '    ax[0].set_title("How strongly, and for whom", fontsize=12)',
    '    if len(pref_all):',
    '        ax[0].axvline(pref_all.mean(), color="#111", lw=2, ls="--")',
    '        ax[0].text(pref_all.mean(), max(counts) * 0.96, " mean %.2f" % pref_all.mean(), fontsize=10, va="top")',
    '    shares = [opus, ties, haiku]',
    '    ax[1].bar(["Opus", "Tie", "Haiku"], shares, color=["#e67e22", "#9a978f", "#3d7bd6"])',
    '    ax[1].set_ylabel("comparisons", fontsize=11)',
    '    ax[1].set_title("Who was preferred (n=%d)" % n, fontsize=12)',
    '    for i, v in enumerate(shares):',
    '        ax[1].text(i, v, "%d\\n%.0f%%" % (v, 100 * v / n), ha="center", va="bottom", fontsize=10)',
    '    fig.tight_layout()',
    '',
    ...
    '        y = range(len(top))',
    '        ax2.barh(list(y), top["mean_pref"].values, color=[col(v) for v in top["mean_pref"].values])',
    '        ax2.set_yticks(list(y)); ax2.set_yticklabels(top.index.astype(str), fontsize=9)',
    '        ax2.invert_yaxis(); ax2.axvline(0, color="#111", lw=1)',
    '        ax2.set_xlabel("mean graded preference  (>0 favours Opus)", fontsize=11)',
    '        ax2.set_title("Which user needs favour Opus vs Haiku", fontsize=12)',
    '        fig2.tight_layout()',
    '',
    '    # Figure 3: mean preference by 2x2 condition, with 95% CI',
    '    if factors:',
    '        fig3, ax3 = plt.subplots(1, len(factors), figsize=(5.2 * len(factors), 4.2), squeeze=False)',
    ...
    '                labs.append("%s=%g\\n(n=%d)" % (c, lvl, len(grp)))',
    '                ms.append(grp["pref"].mean())',
    '                es.append(1.96 * grp["pref"].std(ddof=1) / np.sqrt(len(grp)) if len(grp) > 1 else 0)',
    '            a.bar(range(len(ms)), ms, yerr=es, capsize=6, color="#e67e22")',
    '            a.set_xticks(range(len(labs))); a.set_xticklabels(labs, fontsize=10)',
    '            a.axhline(0, color="#111", lw=1); a.set_title("Mean preference by %s" % c, fontsize=12)',
    '            a.set_ylabel("mean graded preference", fontsize=11)',
    '        fig3.tight_layout()',
    '',
    '    # ── INSIGHTS (plain language; rendered by the Insights section) ────────────',
    '    # Headline uses the task-level average (the "averaging across tasks" answer);',
    '    # falls back to the comparison-level mean if there are too few tasks.',
    '    head_mean = mtm if k_tasks >= 2 else mean',
    '    head_p = p_task if k_tasks >= 2 else p_mean',
    '    who = "Opus" if head_mean > 0 else ("Haiku" if head_mean < 0 else "neither model")',
    '    sig = (not np.isnan(head_p)) and head_p < 0.05',
    '    strength = "clear" if abs(head_mean) >= 0.5 else ("modest" if abs(head_mean) >= 0.15 else "slight")',
    '    print("\\n\\nINSIGHTS")',
    '    print("=" * 72)',
    '    note("## Do users prefer a model, or are they indifferent?")',
    '    if np.isnan(head_mean):',
    '        note("- Not enough graded data to summarise yet.")',
    '    elif sig:',
    '        note("- **We can reject indifference: users prefer %s.** Averaging across the %d tasks, the "',
    '             "mean graded preference is **%+.2f** (positive favours Opus) - a **%s** lean - and a "',
    '             "one-sample t-test across tasks rejects \\"no preference\\" (**p = %.3g**)."',
    '             % (who, k_tasks, head_mean, strength, head_p))',
    '    else:',
    '        note("- **We cannot reject indifference.** Averaging across the %d tasks the mean graded "',
    '             "preference is **%+.2f** (positive favours Opus), but a one-sample t-test across tasks "',
    '             "does **not** distinguish it from zero (**p = %.3g**) - no clear overall winner."',
    '             % (k_tasks, head_mean, head_p))',
    '    if not np.isnan(mean):',
    '        note("- Cross-check across all comparisons (clustered on participant): mean **%+.2f**, "',
    '             "95%% CI [%.2f, %.2f], p = %.3g%s."',
    '             % (mean, lo, hi, p_mean, " (agrees)" if (not np.isnan(p_mean) and (p_mean < 0.05) == sig) else ""))',
    '    if pos + neg:',
    '        note("- Sign test: **%d of %d** tasks lean toward %s (p = %.3g)."',
    '             % (max(pos, neg), pos + neg, "Opus" if pos >= neg else "Haiku", p_sign))',
    '    note("")',
    '    note("## How often each answer won")',
    '    if decisive:',
    '        note("- Ignoring ties, **Opus was preferred in %.0f%%** of the %d decisive comparisons "',
    '             "(Haiku %.0f%%); binomial test vs 50/50 p = %.3g." % (100 * win, decisive, 100 - 100 * win, p_bin))',
    '    note("- Ties (\\"equally good\\") were **%.0f%%** of all %d comparisons." % (100 * ties / n, n))',
    '    note("")',
    '    note("## It depends on the task")',
    '    if tasks_total:',
    '        note("- Across the %d tasks, Opus had the higher mean preference on **%d**, Haiku on **%d** "',
  ...
  var DA_R_TEMPLATE = [
    ...
    '#',
    '# Each row of the Responses table is one BLIND comparison: a participant saw a',
    '# task (a real user need) and two answers - one from Haiku (baseline), one from',
    '# Opus (frontier) - without being told which was which, chose the one they',
    '# preferred (or "equally good"), then graded how strongly on a 7-point scale.',
    '#',
    '# The picked table is mounted at /tmp/data.csv. Key columns: account_id (cluster',
    '# unit), task_id, chosen_model (baseline/frontier/tie), preference_model',
    '# (-3..+3; <0 Haiku, >0 Opus), cost_transparency/firm_pay (1/0), submitted.',
    '# Edit anything, then Run. The final INSIGHTS block feeds the Insights section.',
    '',
    ...
    'df$pref <- if ("preference_model" %in% names(df)) num(df$preference_model) else NA_real_',
    'df$chosen <- if ("chosen_model" %in% names(df)) tolower(as.character(df$chosen_model)) else ""',
    '',
    'n <- nrow(df)',
    'np <- if ("account_id" %in% names(df)) length(unique(df$account_id)) else NA',
    'nt <- if ("task_id" %in% names(df)) length(unique(df$task_id)) else NA',
    'cat(strrep("=", 72), "\\n")',
    'cat("ANSWER ARENA - blind preference: Haiku 4.5 (baseline) vs Opus 4.8 (frontier)\\n")',
    'cat(strrep("=", 72), "\\n")',
    'cat(sprintf("Comparisons: %d   Participants: %s   Tasks: %s\\n", n, as.character(np), as.character(nt)))',
    '',
    'INS <- character(0)',
    'add <- function(s = "") INS <<- c(INS, s)',
    '',
    '# Cluster-robust vcov (CR1) for an lm; falls back to HC3 with a single cluster.',
    'cluster_vcov <- function(fit, cl) {',
    '  X <- model.matrix(fit); u <- residuals(fit); k <- ncol(X); nn <- nrow(X)',
    '  bread <- solve(t(X) %*% X); uj <- X * u',
    '  cl <- factor(cl); G <- nlevels(cl)',
    '  if (G < 2) { h <- hatvalues(fit); meat <- t(X) %*% (X * ((u^2) / (1 - h)^2)); return(bread %*% meat %*% bread) }',
    '  meat <- matrix(0, k, k)',
    '  for (g in levels(cl)) { s <- colSums(uj[cl == g, , drop = FALSE]); meat <- meat + outer(s, s) }',
    '  adj <- (G / (G - 1)) * ((nn - 1) / (nn - k))',
    '  bread %*% (adj * meat) %*% bread',
    '}',
    'coef_table <- function(fit, cl) {',
    '  V <- cluster_vcov(fit, cl); est <- coef(fit); se <- sqrt(diag(V))',
    '  tval <- est / se; p <- 2 * pt(-abs(tval), df.residual(fit))',
    '  data.frame(coef = est, robust_SE = se, t = tval, p = p)',
    '}',
    '',
    'if (n == 0) {',
    '  cat("\\nNo comparisons in the selected table - load the Responses table in Section 1.\\n")',
    '} else {',
    '  # TABLE 1 - overall outcome shares',
    '  opus <- sum(df$chosen == "frontier"); haiku <- sum(df$chosen == "baseline"); ties <- sum(df$chosen == "tie")',
    '  decisive <- opus + haiku; win <- if (decisive > 0) opus / decisive else NA',
    '  t1 <- data.frame(outcome = c("Opus preferred", "Haiku preferred", "Equally good (tie)"),',
    '                   n = c(opus, haiku, ties), share_pct = round(100 * c(opus, haiku, ties) / n, 1))',
    '  cat("\\nTABLE 1 - Overall outcome shares\\n"); print(t1, row.names = FALSE)',
    '  p_bin <- NA',
    '  if (decisive > 0) {',
    '    p_bin <- binom.test(opus, decisive, 0.5)$p.value',
    '    cat(sprintf("\\nAmong the %d decisive comparisons (ties dropped), Opus was preferred %d times = %.1f%%.\\nTwo-sided binomial test vs an even 50/50 split: p = %.4g.\\n", decisive, opus, 100 * win, p_bin))',
    '  }',
    '',
    '  # THE MAIN QUESTION - are users INDIFFERENT (mean preference = 0), or do they',
    '  # prefer a model (mean != 0; >0 Opus, <0 Haiku)? Answered two ways:',
    '  # (A) averaging ACROSS TASKS - each task\'s mean is one observation (so tasks',
    '  #     with more comparisons don\'t dominate; the headline test), one-sample',
    '  #     t-test vs 0; and (B) across all comparisons, clustered on participant.',
    '  reg <- df[!is.na(df$pref), , drop = FALSE]',
    '',
    '  # (A) task-level one-sample t-test',
    '  task_means <- if ("task_id" %in% names(df)) tapply(df$pref, df$task_id, function(x) mean(x, na.rm = TRUE)) else numeric(0)',
    '  task_means <- task_means[!is.na(task_means)]',
    '  k_tasks <- length(task_means); mtm <- t_task <- p_task <- tlo <- thi <- NA',
    '  if (k_tasks >= 2) {',
    '    tr <- t.test(task_means, mu = 0)',
    '    mtm <- unname(tr$estimate); t_task <- unname(tr$statistic); p_task <- tr$p.value',
    '    tlo <- tr$conf.int[1]; thi <- tr$conf.int[2]',
    '  }',
    '  pos <- sum(task_means > 0); neg <- sum(task_means < 0)',
    '  p_sign <- if (pos + neg > 0) binom.test(pos, pos + neg, 0.5)$p.value else NA',
    '',
    '  # (B) comparison-level intercept-only OLS, clustered on participant',
    '  meanp <- se <- pmean <- lo <- hi <- NA',
    '  if (nrow(reg) > 0) {',
    '    fit0 <- lm(pref ~ 1, data = reg)',
    '    cl0 <- if ("account_id" %in% names(reg)) reg$account_id else seq_len(nrow(reg))',
    '    ct <- coef_table(fit0, cl0)',
    '    meanp <- ct$coef[1]; se <- ct$robust_SE[1]; pmean <- ct$p[1]',
    '    lo <- meanp - 1.96 * se; hi <- meanp + 1.96 * se',
    '  }',
    '  fmtci <- function(a, b) if (is.na(a)) "n/a" else sprintf("[%.3f, %.3f]", a, b)',
    '  cat("\\nTABLE 2 - The main test: are users indifferent (mean preference = 0)?\\n")',
    '  cat("  -3 = strongly Haiku .. 0 = equal .. +3 = strongly Opus\\n")',
    '  t2 <- data.frame(',
    '    test = c("(A) averaging across tasks (task = unit)", "(B) all comparisons, clustered on participant"),',
    '    n = c(k_tasks, nrow(reg)),',
    '    mean_pref = round(c(mtm, meanp), 4),',
    '    CI95 = c(fmtci(tlo, thi), fmtci(lo, hi)),',
    '    p_mean0 = round(c(p_task, pmean), 4))',
    '  print(t2, row.names = FALSE)',
    '  if (k_tasks >= 2) cat(sprintf("  (A) one-sample t-test across %d tasks: t(%d) = %.3f, p = %.4g\\n", k_tasks, k_tasks - 1, t_task, p_task))',
    '  if (pos + neg > 0) cat(sprintf("  Sign test across tasks: %d favour Opus, %d favour Haiku (two-sided p = %.4g)\\n", pos, neg, p_sign))',
    '',
    '  # TABLE 3 - regression on the 2x2 factors (if varied)',
    '  factors <- intersect(c("cost_transparency", "firm_pay"), names(df))',
    '  for (fc in factors) reg[[fc]] <- num(reg[[fc]])',
    '  # vapply (not sapply) so an empty `factors` yields logical(0), not list()',
    '  factors <- factors[vapply(factors, function(fc) length(unique(reg[[fc]][!is.na(reg[[fc]])])) > 1, logical(1))]',
    '  m2 <- NULL',
    '  if (length(factors) && nrow(reg[stats::complete.cases(reg[, c("pref", factors)]), , drop = FALSE]) > length(factors) + 1) {',
    '    rr <- reg[stats::complete.cases(reg[, c("pref", factors)]), , drop = FALSE]',
    '    m2 <- tryCatch({',
    '      fit <- lm(as.formula(paste("pref ~", paste(factors, collapse = " + "))), data = rr)',
    '      clv <- if ("account_id" %in% names(rr)) rr$account_id else seq_len(nrow(rr))',
    '      cth <- coef_table(fit, clv)',
    '      cat("\\nTABLE 3 - OLS  pref ~", paste(factors, collapse = " + "), " (SEs clustered on participant)\\n")',
    '      print(round(cth, 4))',
    '      cth',
    '    }, error = function(e) { cat("\\n(Factor regression skipped:", conditionMessage(e), ")\\n"); NULL })',
    '  }',
    '',
    '  # TABLE 4 - by task (heterogeneity across user needs)',
    '  tasks_total <- t_opus <- t_haiku <- 0; tt <- NULL',
    '  if ("task_id" %in% names(df)) {',
    '    ids <- unique(df$task_id)',
    '    tt <- data.frame(task_id = ids,',
    '                     n = sapply(ids, function(i) sum(df$task_id == i)),',
    '                     opus = sapply(ids, function(i) sum(df$task_id == i & df$chosen == "frontier")),',
    '                     haiku = sapply(ids, function(i) sum(df$task_id == i & df$chosen == "baseline")),',
    '                     tie = sapply(ids, function(i) sum(df$task_id == i & df$chosen == "tie")),',
    '                     mean_pref = sapply(ids, function(i) mean(df$pref[df$task_id == i], na.rm = TRUE)),',
    '                     stringsAsFactors = FALSE)',
    '    dec <- tt$opus + tt$haiku',
    '    tt$opus_win_pct <- ifelse(dec > 0, round(100 * tt$opus / dec, 1), NA)',
    '    tt$mean_pref <- round(tt$mean_pref, 2)',
    '    tt <- tt[order(-tt$mean_pref), ]',
    '    tasks_total <- nrow(tt); t_opus <- sum(tt$mean_pref > 0, na.rm = TRUE); t_haiku <- sum(tt$mean_pref < 0, na.rm = TRUE)',
    '    cat("\\nTABLE 4 - By task  (sorted by mean preference; >0 favours Opus)\\n"); print(tt, row.names = FALSE)',
    '  }',
    '',
    '  # PLOTS',
    '  colf <- function(v) ifelse(v > 0, "#e67e22", ifelse(v < 0, "#3d7bd6", "#9a978f"))',
    '  # Figure 1: distribution of graded preference + outcome shares',
    '  op <- par(mfrow = c(1, 2))',
    '  pv <- df$pref[!is.na(df$pref)]',
    '  cnt <- sapply(-3:3, function(v) sum(pv == v))',
    '  barplot(cnt, names.arg = -3:3, col = colf(-3:3),',
    '          xlab = "graded preference (-3 Haiku .. +3 Opus)", ylab = "comparisons", main = "How strongly, and for whom")',
    '  barplot(c(opus, ties, haiku), names.arg = c("Opus", "Tie", "Haiku"), col = c("#e67e22", "#9a978f", "#3d7bd6"),',
    '          ylab = "comparisons", main = sprintf("Who was preferred (n=%d)", n))',
    '  par(op)',
    '  # Figure 2: mean preference by task',
    '  if (!is.null(tt) && tasks_total) {',
    '    o <- order(tt$mean_pref)',
    '    op2 <- par(mar = c(4.5, 6, 3, 1))',
    '    barplot(tt$mean_pref[o], names.arg = tt$task_id[o], horiz = TRUE, las = 1, col = colf(tt$mean_pref[o]),',
    '            xlab = "mean graded preference (>0 favours Opus)", main = "Which user needs favour Opus vs Haiku")',
    '    abline(v = 0)',
    '    par(op2)',
    '  }',
    '  # Figure 3: mean preference by each 2x2 factor, with 95% CI',
    '  if (length(factors)) {',
    '    op3 <- par(mfrow = c(1, length(factors)))',
    '    for (fc in factors) {',
    '      sub <- reg[!is.na(reg[[fc]]), ]',
    '      lv <- sort(unique(sub[[fc]]))',
    '      ms <- sapply(lv, function(l) mean(sub$pref[sub[[fc]] == l]))',
    '      es <- sapply(lv, function(l) { x <- sub$pref[sub[[fc]] == l]; if (length(x) > 1) 1.96 * sd(x) / sqrt(length(x)) else 0 })',
    '      bp <- barplot(ms, names.arg = sprintf("%s=%g", fc, lv), col = "#e67e22", ylim = range(0, ms + es, ms - es),',
    '                    ylab = "mean graded preference", main = sprintf("Mean preference by %s", fc))',
    '      arrows(bp, ms - es, bp, ms + es, angle = 90, code = 3, length = 0.05)',
    '      abline(h = 0)',
    '    }',
    '    par(op3)',
    '  }',
    '',
    '  # INSIGHTS (plain language; rendered by the Insights section). Headline uses the',
    '  # task-level average (the "averaging across tasks" answer); falls back to the',
    '  # comparison-level mean when there are too few tasks.',
    '  head_mean <- if (k_tasks >= 2) mtm else meanp',
    '  head_p <- if (k_tasks >= 2) p_task else pmean',
    '  who <- if (is.na(head_mean) || head_mean == 0) "neither model" else if (head_mean > 0) "Opus" else "Haiku"',
    '  sig <- !is.na(head_p) && head_p < 0.05',
    '  strength <- if (!is.na(head_mean) && abs(head_mean) >= 0.5) "clear" else if (!is.na(head_mean) && abs(head_mean) >= 0.15) "modest" else "slight"',
    '  cat("\\n\\nINSIGHTS\\n"); cat(strrep("=", 72), "\\n")',
    '  add("## Do users prefer a model, or are they indifferent?")',
    '  if (is.na(head_mean)) {',
    '    add("- Not enough graded data to summarise yet.")',
    '  } else if (sig) {',
    '    add(sprintf("- **We can reject indifference: users prefer %s.** Averaging across the %d tasks, the mean graded preference is **%+.2f** (positive favours Opus) - a **%s** lean - and a one-sample t-test across tasks rejects \\"no preference\\" (**p = %.3g**).", who, k_tasks, head_mean, strength, head_p))',
    '  } else {',
    '    add(sprintf("- **We cannot reject indifference.** Averaging across the %d tasks the mean graded preference is **%+.2f** (positive favours Opus), but a one-sample t-test across tasks does **not** distinguish it from zero (**p = %.3g**) - no clear overall winner.", k_tasks, head_mean, head_p))',
    '  }',
    '  if (!is.na(meanp)) add(sprintf("- Cross-check across all comparisons (clustered on participant): mean **%+.2f**, 95%% CI [%.2f, %.2f], p = %.3g%s.", meanp, lo, hi, pmean, if (!is.na(pmean) && (pmean < 0.05) == sig) " (agrees)" else ""))',
    '  if (pos + neg > 0) add(sprintf("- Sign test: **%d of %d** tasks lean toward %s (p = %.3g).", max(pos, neg), pos + neg, if (pos >= neg) "Opus" else "Haiku", p_sign))',
    '  add("")',
    '  add("## How often each answer won")',
    '  if (decisive > 0) add(sprintf("- Ignoring ties, **Opus was preferred in %.0f%%** of the %d decisive comparisons (Haiku %.0f%%); binomial test vs 50/50 p = %.3g.", 100 * win, decisive, 100 - 100 * win, p_bin))',
    '  add(sprintf("- Ties (\\"equally good\\") were **%.0f%%** of all %d comparisons.", 100 * ties / n, n))',
    '  add("")',
    '  add("## It depends on the task")',
    '  if (tasks_total) add(sprintf("- Across the %d tasks, Opus had the higher mean preference on **%d**, Haiku on **%d** (the rest tied) - so the winner is **not uniform** across user needs (see the by-task table and plot).", tasks_total, t_opus, t_haiku))',
    '  if (length(factors) && !is.null(m2)) {',
    '    add(""); add("## The 2x2 conditions")',
    '    for (fc in factors) {',
    '      b <- m2[fc, "coef"]; pp <- m2[fc, "p"]',
    '      add(sprintf("- Turning on **%s** %s the preference for Opus by **%+.2f** points (p = %.3g)%s.", fc, if (b > 0) "raises" else "lowers", b, pp, if (pp < 0.05) "" else ", not significant"))',
    '    }',
    '  }',
    '  for (s in INS) cat(s, "\\n")',
    '  cat("\\nDone.\\n")',
