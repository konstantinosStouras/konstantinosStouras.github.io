...
(function () {
  ...
  var DA_TPL_VERSION = '2026-07-07-stats-review';
  ...
  var DA_PY_TEMPLATE = [
    ...
    '    return {"beta": beta, "se": se, "t": t, "p": p, "n": n, "df": dfree}',
    '',
    '',
    ...
    'df["chosen"] = df["chosen_model"].astype(str).str.lower() if "chosen_model" in df.columns else ""  # baseline/frontier/tie',
    'df["task_id"] = df["task_id"].astype(str) if "task_id" in df.columns else ""     # join key as text',
    '',
  ...
  var DA_R_TEMPLATE = [
    ...
    '    # ("data are essentially constant"). Match scipy instead of crashing:',
    '    # every answer identical and != mu -> t = +/-Inf, p = 0; identical == mu -> NA.',
    '    if (mean(x) == mu) return(list(t = NA, p = NA, df = length(x) - 1))',
    ...
    '  tval <- beta / se; p <- 2 * pt(-abs(tval), dfree)      # t stats and two-sided p',
    '  list(beta = beta, se = se, t = tval, p = p, n = n, df = dfree)',
    '}',
    ...
    'if ("submitted" %in% names(df)) df <- df[tolower(as.character(df$submitted)) %in% "yes", , drop = FALSE]',
    '# Each column is optional-safe: a table without it just yields empty values',
    '# instead of crashing (e.g. running the script on the Events sheet).',
    ...
    'df$chosen <- if ("chosen_model" %in% names(df)) tolower(as.character(df$chosen_model)) else rep("", nrow(df))   # baseline/frontier/tie',
    'df$task_id <- if ("task_id" %in% names(df)) as.character(df$task_id) else rep("", nrow(df))                     # join key as text',
    '',
