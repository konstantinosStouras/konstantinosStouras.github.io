...
(function () {
  ...
  var DA_TPL_VERSION = '2026-08-12-conf99';
  ...
  var DA_PY_TEMPLATE = [
    ...
    '',
    '',
    'def decide(m, se, dfree):',
    '    """Classify ONE estimate - a mean `m` with standard error `se` and `dfree`',
    '    degrees of freedom - into exactly one of the four Section-1 verdicts, at',
    '    CONF_LEVEL. Returns a dict with the interval, both p-values and the verdict:',
    '      p_diff   two-sided t-test of the mean vs 0   -> "opus"/"haiku" if < ALPHA',
    '      p_equiv  TOST equivalence vs +/-EQUIV_MARGIN -> "indifferent" if < ALPHA',
    ...
    '    else:',
    '        tc = float(st.t.ppf(0.5 + CONF_LEVEL / 2, dfree))    # 99% t critical value',
    '        out["lo"], out["hi"] = m - tc * se, m + tc * se      # the 99% interval',
  ...
  var DA_R_TEMPLATE = [
    ...
    '# of the four Section-1 verdicts at CONF_LEVEL. Same rules as the Python `decide`:',
    '#   p_diff  two-sided t-test of the mean vs 0   -> "opus"/"haiku" when < ALPHA',
    '#   p_equiv TOST equivalence vs +/-EQUIV_MARGIN -> "indifferent" when < ALPHA',
    ...
    '  } else {',
    '    tc <- qt(0.5 + CONF_LEVEL / 2, dfree)                 # 99% t critical value',
    '    out$lo <- m - tc * se; out$hi <- m + tc * se          # the 99% interval',
