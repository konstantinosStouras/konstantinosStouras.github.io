...
(function () {
  ...
  var DA_TPL_VERSION = '2026-08-12-conf99-sets95';
  ...
  function renderAnalytics() {
    ...
    wrap.appendChild(el('div', { class: 'aa-card' }, [
      ...
      el('p', { class: 'aa-note', html: 'Load your session data (or import an already-exported Excel), consolidate it into a single workbook, then run Python or R on it — compiled entirely in your browser (nothing is uploaded). Each comparison asked a blind participant which answer they preferred (Haiku vs Opus, unlabelled) and how strongly, so the bundled scripts open by classifying every task, task type and domain into <b>Haiku preferred</b> / <b>Opus preferred</b> / <b>indifferent</b> / <b>not decided yet</b> — each claim held to <b>99% confidence</b>. Four steps:' })
    ]));
  ...
  var DA_PY_TEMPLATE = [
    ...
    '  1. THE HEADLINE, at 99% confidence: which tasks we can be confident Haiku is',
    ...
    '  8. The same classification as 1, but at 95% confidence: the three sets of tasks',
    '     whose true preference we can name (Haiku / Opus / indifferent), with the',
    '     unclassifiable ones deliberately left out.',
    ...
    '',
    '# ── Settings for Section 1, the 99%-confidence classification ────────────────',
    '# Section 1 answers the practical question - for THIS task, is the frontier model',
    '# needed? - at a demanding 99% confidence. THREE different claims are possible and',
    '# each needs its own test:',
    '#   "Haiku preferred" / "Opus preferred": the two-sided t-test of that task\'s mean',
    '#       graded preference against 0 is significant at the 1% level - equivalently,',
    '#       its 99% confidence interval sits entirely on one side of 0.',
    ...
    '#       positive EQUIVALENCE test - TOST, two one-sided t-tests each at the 1%',
    '#       level, which is significant exactly when the 98% CI lies inside the margin.',
    '#   "Not decided yet": neither claim reached 99% - more responses are needed.',
    'CONF_LEVEL = 0.99                # confidence demanded by every Section-1 verdict',
    'ALPHA = 1 - CONF_LEVEL           # 0.01 - the level each of those tests is run at',
    ...
    '    `conf` (99% for Section 1, 95% for the confident sets in Section 8).',
    ...
    'else:',
    '    # =====================================================================',
    '    # 1. WHAT WE CAN SAY WITH 99% CONFIDENCE  (the headline, printed first)',
    '    # =====================================================================',
    ...
    '                          "99%% CI still spans 0, and too wide to prove |mean| < %.2f" % EQUIV_MARGIN)',
    '',
    '    print("\\n" + "=" * 74)',
    '    print("1. WHAT WE CAN SAY WITH 99% CONFIDENCE")',
    '    print("=" * 74)',
    '    print("   Every verdict below is made at the 1% level (99% confidence) on the graded")',
    '    print("   preference (-3 = Haiku much better .. 0 = equivalent .. +3 = Opus much better):")',
    '    print("     Haiku preferred   the task\'s 99% CI for the mean lies entirely BELOW 0")',
    '    print("     Opus preferred    ... lies entirely ABOVE 0")',
    '    print("     Indifferent       an EQUIVALENCE test (TOST: two one-sided t-tests at 1%)")',
    ...
    '    print("     Not decided yet   neither claim reached 99% - more responses are needed.")',
    '',
    ...
    '        return sub',
    '',
    '    c99_haiku = c99_block("haiku", "1a. HAIKU (the smaller model) is preferred, at 99% confidence",',
    '                          "Buying the frontier model here is over-provisioning: users liked the small one MORE.")',
    '    c99_opus = c99_block("opus", "1b. OPUS (the frontier model) is preferred, at 99% confidence",',
    '                         "Serving these with the small model is under-provisioning: users noticed the difference.")',
    '    c99_ind = c99_block("indifferent", "1c. USERS ARE INDIFFERENT, at 99% confidence (equivalence test)",',
    '                        "Proven equivalence, not just a null result - so the cheaper model is the right call.")',
    '    c99_und = c99_block("undecided", "1d. NOT DECIDED YET at 99% confidence",',
    ...
    '    def group99(col):',
    '        """One row per group: the equal-weight average of its task means, its 99%',
    ...
    '    print("\\n" + "-" * 74)',
    '    print("1e. BY TASK TYPE, at 99% confidence (averaged across that type\'s tasks)")',
    '    print("-" * 74)',
    '    print(type99.to_string(index=False, float_format=lambda x: "%.3f" % x))',
    '    print("\\n" + "-" * 74)',
    '    print("1f. BY DOMAIN, at 99% confidence (averaged across that domain\'s tasks)")',
    '    print("-" * 74)',
    ...
    '',
    '    # Figure 1: the 99% verdict for every task. Bar = the task\'s mean graded',
    '    # preference, whisker = its 99% CI, colour = the verdict, and the shaded band is',
    '    # the +/-EQUIV_MARGIN indifference zone a task must fit INSIDE (with 99%',
    '    # confidence) to be called indifferent. Blocks in the printed order.',
    ...
    '    ax99.set_xlabel("mean graded preference with its 99% CI  (<0 Haiku .. >0 Opus)")',
    '    ax99.set_title("What we can say at 99% confidence, per task")',
    ...
    '',
    '    # =====================================================================',
    '    # 8. THE 95%-CONFIDENT SETS  (what the ground truth is, where we can say)',
    '    # =====================================================================',
    '    # The same three tests as Section 1, run at 95% instead of 99%: a task joins a',
    '    # set only when its own responses put it there with 95% confidence. "Ground',
    '    # truth" here means the preference of the POPULATION these students are drawn',
    '    # from - what everyone would say, not just the students who happened to answer',
    '    # - which is exactly what a confidence interval is a claim about.',
    ...
    '    s95_rows = []',
    '    for t, g in df.groupby("task_id"):',
    '        nn, m, sd, se = sample_stats(g["pref"].values)',
    '        d = decide(m, se, (nn - 1) if nn >= 2 else np.nan, CONF95)',
    '        s95_rows.append({"task_id": t, "complexity": g["complexity"].iloc[0],',
    '                         "domain": g["domain"].iloc[0], "n": nn, "mean_pref": m,',
    '                         "ci_lo": clip3(d["lo"]), "ci_hi": clip3(d["hi"]),',
    '                         "p_diff": d["p_diff"], "p_equiv": d["p_equiv"], "verdict": d["verdict"]})',
    '    s95 = pd.DataFrame(s95_rows, columns=["task_id", "complexity", "domain", "n", "mean_pref",',
    '                       "ci_lo", "ci_hi", "p_diff", "p_equiv", "verdict"])',
    '    print("\\n" + "=" * 74)',
    '    print("8. THE 95%-CONFIDENT SETS")',
    '    print("=" * 74)',
    '    print("   The same three tests as Section 1, run at the 5% level instead of the 1%")',
    '    print("   one, so each set below is the tasks whose true (population) preference we")',
    '    print("   can name with 95% confidence. Intervals shown are 95% intervals.")',
    '    print("   Tasks that reach no 95% verdict are NOT listed - that is an absence of")',
    '    print("   evidence, not a third kind of answer.")',
    '',
    '    def set95(key, title, gloss):',
    '        """Print one 95%-confident set. Strongest evidence first (closest to 0 first',
    '        for the indifferent set, since there the claim is that the gap is small)."""',
    '        sub = s95[s95["verdict"] == key].copy()',
    '        if key == "haiku":',
    '            sub = sub.sort_values("mean_pref", ascending=True)',
    '        elif key == "indifferent":',
    '            sub = sub.assign(_abs=sub["mean_pref"].abs()).sort_values("_abs", ascending=True)',
    ...
    '        print("-" * 74)',
    '        if not len(sub):',
    '            print("   (none reach 95% confidence)")',
    '            return sub',
    '        cols = ["task_id", "complexity", "domain", "n", "mean_pref", "ci_lo", "ci_hi", "p_diff", "p_equiv"]',
    '        print(sub[cols].to_string(index=False, float_format=lambda x: "%.3f" % x))',
    '        return sub',
    '',
    '    s95_haiku = set95("haiku", "8a. GROUND TRUTH = PEOPLE PREFER HAIKU (95% confidence)",',
    '                      "The whole 95% interval sits below 0: the smaller model\'s answer won.")',
    '    s95_opus = set95("opus", "8b. GROUND TRUTH = PEOPLE PREFER OPUS (95% confidence)",',
    '                     "The whole 95% interval sits above 0: the frontier model\'s answer won.")',
    '    s95_ind = set95("indifferent", "8c. GROUND TRUTH = PEOPLE ARE INDIFFERENT (95% confidence)",',
    '                    "An equivalence test puts the true gap inside +/-%.2f - proven, not just unproven." % EQUIV_MARGIN)',
    '    n95_in = len(s95_haiku) + len(s95_opus) + len(s95_ind)',
    '    print("\\n   %d of the %d tasks are classified at 95%%: %d Haiku, %d Opus, %d indifferent."',
    '          % (n95_in, len(s95), len(s95_haiku), len(s95_opus), len(s95_ind)))',
    '    print("   The remaining %d reach no 95%% verdict and are deliberately not listed."' ,
    '          % (len(s95) - n95_in))',
    '    # Section 1 answers the same question at 99%, so its sets are SUBSETS of these:',
    '    # a stricter bar can only classify fewer tasks, never different ones.',
    '    print("   Section 1 asks the same question at 99%: its sets are subsets of these.")',
    '',
    '    # =====================================================================',
    '    # PLOTS',
    '    # =====================================================================',
    ...
    '    note("## Headline - what we can say at 99% confidence")',
    '    note("- Of the **%d tasks**, at 99%% confidence users **prefer Haiku on %d**, **prefer Opus on "',
    '         "%d**, are **provably indifferent on %d**, and **%d are not decided yet**."',
    '         % (len(c99), len(c99_haiku), len(c99_opus), len(c99_ind), len(c99_und)))',
    ...
    '         "absence of a finding** - neither a preference nor equivalence reached 99%%, so those tasks "',
    '         "simply need more responses." % EQUIV_MARGIN)',
    '    for _, r in type99.iterrows():',
    '        note("- **%s** tasks, averaged across their %d tasks: mean **%s** (99%% CI [%s, %s]) -> "',
    '             "**%s** (of those tasks: %d Haiku, %d Opus, %d indifferent, %d undecided)."',
    '             % (r["complexity"], r["n_tasks"], fmt2(r["mean_pref"]), fmt2(r["ci_lo"]), fmt2(r["ci_hi"]),',
    ...
    '            note("- Domain **%s**: mean **%s** (99%% CI [%s, %s]) -> **%s**."',
    '                 % (r["domain"], fmt2(r["mean_pref"]), fmt2(r["ci_lo"]), fmt2(r["ci_hi"]), r["verdict"]))',
    '        if len(dom99) > len(dom_call):',
    '            note("- The remaining %d domain(s) cannot be called either way at 99%% yet."',
    '                 % (len(dom99) - len(dom_call)))',
    '    else:',
    '        note("- **No domain** can be called either way at 99% yet (each holds only a handful of "',
    ...
    '    note("")',
    '    note("## The 95%-confident sets (Section 8)")',
    '    note("- Loosening the bar from 99%% to 95%% classifies **%d of the %d tasks**: **%d where "',
    '         "people prefer Haiku**, **%d where people prefer Opus**, and **%d where people are "',
    '         "provably indifferent**. The other %d are left unlisted on purpose - no verdict is not "',
    '         "a verdict." % (n95_in, len(s95), len(s95_haiku), len(s95_opus), len(s95_ind), len(s95) - n95_in))',
    '    note("- Use these when you want the widest picture you can still defend; Section 1\'s 99%% "',
    '         "sets are subsets of them, so anything claimed there is claimed here too. Nothing else "',
    '         "in this script changes - only the confidence demanded of each task.")',
    '    note("")',
    ...
    '    note("")',
    '    note("## Figure 1 - What we can say at 99% confidence, per task")',
    '    note("- The picture of Section 1. Each bar is one task\'s **average** graded preference (left = "',
    '         "Haiku, right = Opus) and its whisker is the **99%% confidence interval** for that average. "',
    ...
    '         "shaded +/-%.2f band), pale grey = **not decided yet**." % EQUIV_MARGIN)',
    ...
    '         "that is why grey is not a result, just a request for more responses. Note grey here is "',
    '         "stricter than grey in Figure 5: this figure demands 99% rather than 95% confidence.")',
    '    note("")',
  ...
  var DA_R_TEMPLATE = [
    ...
    '',
    '# -- Section-1 helpers: the three tests behind every 99% verdict --------------',
    ...
    '# of the four verdicts, at confidence `conf` (99% for Section 1, 95% for the',
    '# confident sets in Section 8). Same rules as the Python `decide`:',
    ...
    '                    sprintf("99%% CI still spans 0, and too wide to prove |mean| < %.2f", EQUIV_MARGIN))',
    '',
    '  cat("\\n", strrep("=", 74), "\\n1. WHAT WE CAN SAY WITH 99% CONFIDENCE\\n", strrep("=", 74), "\\n", sep = "")',
    '  cat("   Every verdict below is made at the 1% level (99% confidence) on the graded\\n")',
    '  cat("   preference (-3 = Haiku much better .. 0 = equivalent .. +3 = Opus much better):\\n")',
    '  cat("     Haiku preferred   the task\'s 99% CI for the mean lies entirely BELOW 0\\n")',
    '  cat("     Opus preferred    ... lies entirely ABOVE 0\\n")',
    '  cat("     Indifferent       an EQUIVALENCE test (TOST: two one-sided t-tests at 1%)\\n")',
    ...
    '  cat("     Not decided yet   neither claim reached 99% - more responses are needed.\\n")',
    '',
    '  # Print one verdict bucket (the four are exhaustive and mutually exclusive).',
    ...
    '',
    '  # --- the same, averaged across the tasks of each task type / each domain ----',
    '  # One row per group: the equal-weight average of its task means, its 99% interval',
    ...
    '  cat("\\n", strrep("-", 74), "\\n1e. BY TASK TYPE, at 99% confidence (averaged across that type\'s tasks)\\n", strrep("-", 74), "\\n", sep = "")',
    '  print(round_df(type99, 3), row.names = FALSE)',
    '  cat("\\n", strrep("-", 74), "\\n1f. BY DOMAIN, at 99% confidence (averaged across that domain\'s tasks)\\n", strrep("-", 74), "\\n", sep = "")',
    ...
    '',
    '  # Figure 1: the 99% verdict for every task. Bar = the task\'s mean graded',
    '  # preference, whisker = its 99% CI, colour = the verdict, and the dotted lines mark',
    '  # the +/-EQUIV_MARGIN indifference zone a task must fit INSIDE (with 99%',
    '  # confidence) to be called indifferent. Blocks in the printed order.',
    ...
    '                  xlab = "mean graded preference with its 99% CI  (<0 Haiku .. >0 Opus)",',
    '                  main = "What we can say at 99% confidence, per task")',
    '  abline(v = c(-EQUIV_MARGIN, EQUIV_MARGIN), col = "#2e9e6b", lty = 3)   # indifference zone',
    '  segments(cf$ci_lo[ri99], bp99, cf$ci_hi[ri99], bp99, col = "#444444")  # 99% whiskers',
    ...
    '  # truth" here means the preference of the POPULATION these students are drawn',
    '  # from - what everyone would say, not just the students who happened to answer -',
    '  # which is exactly what a confidence interval is a claim about. Tasks that reach',
    '  # no verdict are DELIBERATELY NOT LISTED: "we cannot say yet" is not a fourth',
    '  # kind of task, it is the absence of an answer.',
    '  CONF95 <- 0.95',
    '  s95 <- do.call(rbind, lapply(split(df, df$task_id), function(g) {',
    '    st9 <- sample_stats(g$pref)',
    '    d9 <- decide(st9$mean, st9$se, if (st9$n >= 2) st9$n - 1 else NA_real_, CONF95)',
    '    data.frame(task_id = g$task_id[1], complexity = g$complexity[1], domain = g$domain[1],',
    '               n = st9$n, mean_pref = st9$mean, ci_lo = clip3(d9$lo), ci_hi = clip3(d9$hi),',
    '               p_diff = d9$p_diff, p_equiv = d9$p_equiv, verdict = d9$verdict,',
    '               stringsAsFactors = FALSE)',
    '  }))',
    '  cat("\\n", strrep("=", 74), "\\n8. THE 95%-CONFIDENT SETS\\n", strrep("=", 74), "\\n", sep = "")',
    '  cat("   The same three tests as Section 1, run at the 5% level instead of the 1%\\n")',
    '  cat("   one, so each set below is the tasks whose true (population) preference we\\n")',
    '  cat("   can name with 95% confidence. Intervals shown are 95% intervals.\\n")',
    '  cat("   Tasks that reach no 95% verdict are NOT listed - that is an absence of\\n")',
    '  cat("   evidence, not a third kind of answer.\\n")',
    '  # Print one 95%-confident set. Strongest evidence first (closest to 0 first for',
    '  # the indifferent set, since there the claim is that the gap is small).',
    '  set95 <- function(key, title, gloss) {',
    '    sb <- s95[s95$verdict == key, , drop = FALSE]',
    '    ord <- if (key == "haiku") order(sb$mean_pref) else if (key == "indifferent") order(abs(sb$mean_pref)) else order(-sb$mean_pref)',
    '    sb <- sb[ord, , drop = FALSE]',
    '    cat("\\n", strrep("-", 74), "\\n", sprintf("%s  -  %d tasks", title, nrow(sb)),',
    '        "\\n   ", gloss, "\\n", strrep("-", 74), "\\n", sep = "")',
    '    if (nrow(sb) == 0) { cat("   (none reach 95% confidence)\\n"); return(invisible(sb)) }',
    '    cols <- c("task_id", "complexity", "domain", "n", "mean_pref", "ci_lo", "ci_hi", "p_diff", "p_equiv")',
    '    print(round_df(sb[, cols], 3), row.names = FALSE)   # 3 dp display, like Python',
    '    invisible(sb)',
    '  }',
    ...
    '  n95_in <- nrow(s95_haiku) + nrow(s95_opus) + nrow(s95_ind)',
    '  cat(sprintf("\\n   %d of the %d tasks are classified at 95%%: %d Haiku, %d Opus, %d indifferent.\\n",',
    '              n95_in, nrow(s95), nrow(s95_haiku), nrow(s95_opus), nrow(s95_ind)))',
    '  cat(sprintf("   The remaining %d reach no 95%% verdict and are deliberately not listed.\\n", nrow(s95) - n95_in))',
    '  # Section 1 answers the same question at 99%, so its sets are SUBSETS of these:',
    '  # a stricter bar can only classify fewer tasks, never different ones.',
    '  cat("   Section 1 asks the same question at 99%: its sets are subsets of these.\\n")',
    '',
    '  # ===========================================================================',
    '  # PLOTS',
    '  # ===========================================================================',
    ...
    '  add("## Headline - what we can say at 99% confidence")',
    '  add(sprintf("- Of the **%d tasks**, at 99%% confidence users **prefer Haiku on %d**, **prefer Opus on %d**, are **provably indifferent on %d**, and **%d are not decided yet**.", nrow(c99), nrow(c99_haiku), nrow(c99_opus), nrow(c99_ind), nrow(c99_und)))',
    '  add(sprintf("- The three claims are not symmetric. **\\"Indifferent\\" is a positive finding**: an equivalence test (TOST) placed the true mean preference inside +/-%.2f scale points, so those tasks can be served by the cheaper model on purpose. **\\"Not decided yet\\" is the absence of a finding** - neither a preference nor equivalence reached 99%%, so those tasks simply need more responses.", EQUIV_MARGIN))',
    '  for (i in seq_len(nrow(type99))) { rr <- type99[i, ]',
    '    add(sprintf("- **%s** tasks, averaged across their %d tasks: mean **%s** (99%% CI [%s, %s]) -> **%s** (of those tasks: %d Haiku, %d Opus, %d indifferent, %d undecided).", rr$complexity, rr$n_tasks, fmt2(rr$mean_pref), fmt2(rr$ci_lo), fmt2(rr$ci_hi), rr$verdict, rr$t_haiku, rr$t_opus, rr$t_indiff, rr$t_undec)) }',
    '  dom_call <- dom99[dom99$verdict != "Not decided yet", , drop = FALSE]',
    '  if (nrow(dom_call)) {',
    '    for (i in seq_len(nrow(dom_call))) { rr <- dom_call[i, ]',
    '      add(sprintf("- Domain **%s**: mean **%s** (99%% CI [%s, %s]) -> **%s**.", rr$domain, fmt2(rr$mean_pref), fmt2(rr$ci_lo), fmt2(rr$ci_hi), rr$verdict)) }',
    '    if (nrow(dom99) > nrow(dom_call)) add(sprintf("- The remaining %d domain(s) cannot be called either way at 99%% yet.", nrow(dom99) - nrow(dom_call)))',
    '  } else {',
    '    add("- **No domain** can be called either way at 99% yet (each holds only a handful of tasks, so a domain-level claim needs more responses than a single task does).")',
    '  }',
    '  add("")',
    ...
    '  add(""); add("## The 95%-confident sets (Section 8)")',
    '  add(sprintf("- Loosening the bar from 99%% to 95%% classifies **%d of the %d tasks**: **%d where people prefer Haiku**, **%d where people prefer Opus**, and **%d where people are provably indifferent**. The other %d are left unlisted on purpose - no verdict is not a verdict.", n95_in, nrow(s95), nrow(s95_haiku), nrow(s95_opus), nrow(s95_ind), nrow(s95) - n95_in))',
    '  add("- Use these when you want the widest picture you can still defend; Section 1\'s 99% sets are subsets of them, so anything claimed there is claimed here too. Nothing else in this script changes - only the confidence demanded of each task.")',
