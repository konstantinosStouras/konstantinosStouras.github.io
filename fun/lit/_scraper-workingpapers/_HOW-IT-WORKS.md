...
`fun/lit/_scraper-workingpapers/` builds the **Working Papers** dataset for
[stouras.com/fun/lit/](https://www.stouras.com/fun/lit/): the **unpublished
working papers / pre-prints** of every author already listed in "The Lit". It
is a sibling of `fun/lit/_scraper/` (the published-catalog pipeline) and
`fun/lit/_scraper-ft50/` (the FT50 catalog), and it writes into
`fun/lit/data-workingpapers/`, which the page merges at runtime exactly like a
...
## Files it writes into `fun/lit/data-workingpapers/`
...
## The page side (`fun/lit/index.html`)
...
```
node fun/lit/_scraper-workingpapers/selftest.mjs
```
...
1. move `fun/lit/data-workingpapers/` to that repo's `data/` and enable Pages;
2. in `fun/lit/index.html`, change **one** constant —
