 ...
 * FT50 list. (fun/lit's vendored, independent copy of the fun/ft50 check.)
 * ===========================================================================
...
async function main() {
  ...
  if (!live) {
    await writeFile(SUMMARY_PATH,
      ...
      '`fun/lit/_scraper-ft50/journals.json` and update it if the FT changed the list.\n', 'utf8');
    process.exit(2);
  }
  ...
  if (advisory) {
    const lines = [
      ...
      ' `fun/lit/_scraper-ft50/journals.json` by hand if the FT actually changed the list:',
      '',
    ];
  ...
  for (const k of added) {
    ...
    if (!resolved) {
      ...
      lines.push(`- **Added to the FT50 list but NOT auto-added:** ${displayName} — its ISSN could not be ` +
        'resolved via Crossref with confidence. Please add it to `fun/lit/_scraper-ft50/journals.json` manually.');
