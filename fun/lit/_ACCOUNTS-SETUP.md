...
sign-in (email/password, Google, Microsoft, and optionally Apple/GitHub) —
the same accounts feature as `/fun/ms/` — so a logged-in visitor can:
...
`fun/lit/index.html`, default `['google', 'microsoft']`) — keep that list in sync
