/*
 * build-data.mjs — the fun/ft50 data pipeline (FT50 research paper browser).
 * ===========================================================================
 * This is the *entire* backend for stouras.com/fun/ft50/. It runs on a GitHub
 * Actions runner (see .github/workflows/ft50-update-data.yml), pulls every
 * article of all 50 journals on the Financial Times "FT50" research-rank list
 * from the Crossref REST API, and writes static JSON into fun/ft50/data/,
 * which GitHub Pages serves from its CDN. No server, no database, no Google.
 *
 * The journal list itself lives in _scraper/journals.json (one entry per
 * journal: key, name, ISSNs, publisher, capability flags). It is checked
 * against the live FT50 list once a year by _scraper/check-ft50-list.mjs
 * (.github/workflows/ft50-check-list.yml), which updates journals.json when
 * the Financial Times adds or removes journals and then dispatches this build,
 * so the dataset follows the list automatically.
 *
 * Per-journal capabilities (mirroring fun/lit):
 ...
 *     committed cache built by fun/lit/_scraper/informs-editors-local.mjs
 ...
 *
 * What it writes into fun/ft50/data/:
...
function mapWork(item, src) {
  ...
  const row = {
    ...
    _rank: pubRank(yearOf(item), volume, issue, item.page, status),
  };
