/*
 * check-ft50-list.mjs — keep _scraper/journals.json in sync with the FT50 list.
 * ===========================================================================
 ...
 * year by .github/workflows/ft50-check-list.yml, or on demand — fetches the
 * live list, diffs it against _scraper/journals.json, and:
 *
 ...
 *   • a human-readable summary is written to _scraper/_ft50-check-summary.md
...
async function main() {
  ...
  if (!live) {
    await writeFile(SUMMARY_PATH,
      ...
      '`fun/ft50/_scraper/journals.json` and update it if the FT changed the list.\n', 'utf8');
    process.exit(2);
  }
  ...
  if (advisory) {
    const lines = [
      ...
      ' `fun/ft50/_scraper/journals.json` by hand if the FT actually changed the list:',
      '',
    ];
  ...
  for (const k of added) {
    ...
    if (!resolved) {
      ...
      lines.push(`- **Added to the FT50 list but NOT auto-added:** ${displayName} — its ISSN could not be ` +
        'resolved via Crossref with confidence. Please add it to `fun/ft50/_scraper/journals.json` manually.');
